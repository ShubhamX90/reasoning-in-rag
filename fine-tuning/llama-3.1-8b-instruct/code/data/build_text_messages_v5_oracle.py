#!/usr/bin/env python3
"""
Build TEXT-MODE message triples (system, user, assistant) for v5.

This variant is ORACLE-aware:

- It still works for standard end-to-end templates that only use:
    {query}, {retrieved_docs}, {per_doc_notes}
- It ALSO supports ORACLE-style user templates that additionally use:
    {conflict_type}
  (NOTE: in ORACLE mode the model is NOT given conflict_reason; it must generate
   its own concise conflict_reason consistent with the gold conflict_type.)

The underlying canonical data (in_jsonl) is expected to have keys:
  - id
  - query
  - retrieved_docs
  - per_doc_notes
  - conflict_type
  - conflict_reason
  - expected_response: { answer, evidence, abstain }

The assistant side is a fully formatted TEXT-MODE answer with:
  <think> ... </think> + FINAL ANSWER + [[END-OF-ANSWER]].
"""
...
def sanitize_no_ranges(text):
    # Replace ranges like d1–d5 / d1-d5 with a comma pair (d1, d5),
    # avoiding illegal ranges per text-contract rules.
...
def build_reasoning_B(arr, conflict_type):
    ...
    if sup or psup:
        if irr:
            ...
            parts.append(
                f"Docs {list_ids(sup+psup)} align on the answer; mechanism: none (agreement)."
            )
    else:
        parts.append(
...
def main():
    ...
    for ex in read_jsonl(args.in_jsonl):
        # Build user message (ORACLE-aware: passes conflict_type ONLY)
        user_msg = user_template.format(
            query=ex.get("query", ""),
            retrieved_docs=json.dumps(
                ex.get("retrieved_docs", []),
                ensure_ascii=False,
                indent=2,
            ),
            per_doc_notes=json.dumps(
                ex.get("per_doc_notes", []),
                ensure_ascii=False,
                indent=2,
            ),
            conflict_type=(ex.get("conflict_type") or ""),
        )

        # Build assistant (TEXT-MODE)
        arr = build_array(ex)
        ...
        if abstain:
            ...
            valid_ids = [
                i
                for i in ev
                if any(d.get("doc_id") == i for d in (ex.get("retrieved_docs") or []))
            ]
    ...
    print(f"[Convert] {args.in_jsonl} → {args.out_jsonl}  kept={kept}, dropped={dropped}")
