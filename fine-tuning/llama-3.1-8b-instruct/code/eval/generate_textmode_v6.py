#!/usr/bin/env python3
"""
TEXT-MODE Generator v6 (final-messages + sentinel + auto length)
----------------------------------------------------------------
- Uses *.text.messages_v6_final.jsonl-style inputs:
    {"id": "...", "messages": [ {role: system}, {role: user}, {role: assistant} ]}
- For generation, it:
    • Overrides the system message with the v6 system prompt file.
...
Example:
  python code/eval/generate_textmode_v6.py \
    --base_model meta-llama/Llama-3.1-8B-Instruct \
    --lora_dir checkpoints/sft_qlora_v6/best_dev_f1 \
    --input_jsonl data/splits/dev.text.messages_v6_final.jsonl \
    --system_prompt_path prompts/sft/system_text_mode_sft.v6.txt \
    --out_jsonl outputs/dev_generations/sft_qlora_v6_run1.raw.jsonl \
...
def _extract_user_content(msgs: List[Dict[str, str]]) -> str:
    """
    Given messages from *.text.messages_v6_final.jsonl, return the user text.
...
def build_messages_from_final(
    ...
    - Override system with v6 system prompt.
...
def estimate_doc_count_from_user(user_txt: str) -> int:
    ...
    which is how {retrieved_docs} is rendered in user_text_mode_sft.v6.txt.
    """
...
def main():
    ...
    ap.add_argument(
        "--input_jsonl",
        required=True,
        help="*.text.messages_v6_final.jsonl (e.g., data/splits/dev.text.messages_v6_final.jsonl)",
    )
    ap.add_argument(
        "--system_prompt_path",
        required=True,
        help="prompts/sft/system_text_mode_sft.v6.txt",
    )
    ...
    if with_msgs == 0:
        print(
            ...
            "Use data/splits/dev.text.messages_v6_final.jsonl."
        )
