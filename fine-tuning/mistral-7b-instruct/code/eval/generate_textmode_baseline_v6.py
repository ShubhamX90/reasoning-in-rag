#!/usr/bin/env python3
"""
TEXT-MODE baseline generator (v6, end-to-end, messages_v6_final)
----------------------------------------------------------------
...
def estimate_doc_count_from_user(user_txt: str) -> int:
    ...
    which is how {retrieved_docs} is rendered in the v6 user templates.
    """
...
def main():
    ...
    ap.add_argument(
        "--input_jsonl",
        required=True,
        help="*.text.messages_v6_final.jsonl (e.g., data/splits/test.text.messages_v6_final.jsonl)",
    )
    ap.add_argument(
        "--system_prompt_path",
        required=True,
        help="v6 system prompt (e.g., prompts/sft/system_text_mode_sft.v6.txt)",
    )
    ...
    if with_msgs == 0:
        print(
            ...
            "Use *.text.messages_v6_final.jsonl built with the v6 builder."
        )
    ...
    print(f"[BASELINE v6] Wrote → {out_path}")
