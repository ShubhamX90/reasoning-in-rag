#!/usr/bin/env python3
"""
TEXT-MODE Generator v5 (ORACLE variant)
---------------------------------------
- Uses *.oracle.text.messages_v5_final.jsonl-style inputs:
    {"id": "...", "messages": [ {role: system}, {role: user}, {role: assistant} ]}
- ORACLE evaluation setting:
    • The user message ALREADY includes the gold conflict_type (provided via the oracle user prompt).
    • For generation, we override the system message with the ORACLE system prompt file.
    • We reuse the user message text exactly as stored.
- The model is typically the SAME fine-tuned checkpoint used for end-to-end,
  but now prompted with ORACLE instructions that include the gold conflict type.
...
Example:
  python code/eval/generate_textmode_v5_oracle.py \
    --base_model Qwen/Qwen2.5-7B-Instruct \
    --lora_dir checkpoints/sft_qlora_qwen_v5.1_run1/best_dev_f1 \
    --input_jsonl data/splits/test_v5.1.oracle.text.messages_v5_final.jsonl \
    --system_prompt_path prompts/sft/system_text_mode_oracle_sft.v5.1.txt \
    --out_jsonl outputs/test_generations_v5.1_qwen_oracle/sft_qlora_qwen_v5.1_run1.test_v5.1_oracle.raw.jsonl \
    --auto_length --max_new_tokens_base 1200 --max_new_tokens_cap 2200 \
    --attn_impl sdpa --load_in_4bit --local_files_only
"""
...
def _extract_user_content(msgs: List[Dict[str, str]]) -> str:
    """
    Given messages from *.oracle.text.messages_v5_final.jsonl, return the user text.
...
def build_messages_from_final_oracle(
    system_txt: str,
    ex: Dict[str, Any],
) -> List[Dict[str, str]]:
    """
    Build chat messages for ORACLE generation from a final-messages example:
      {"id": "...", "messages": [...]}

    - Override system with ORACLE system prompt.
    - Keep user content as stored (this already includes gold conflict_type).
...
def estimate_doc_count_from_user(user_txt: str) -> int:
    ...
    which is how {retrieved_docs} is rendered in the user templates.
    """
...
def main():
    ...
    ap.add_argument(
        "--input_jsonl",
        required=True,
        help="*.oracle.text.messages_v5_final.jsonl (e.g., data/splits/test_v5.1.oracle.text.messages_v5_final.jsonl)",
    )
    ap.add_argument(
        "--system_prompt_path",
        required=True,
        help="ORACLE system prompt (e.g., prompts/sft/system_text_mode_oracle_sft.v5.1.txt)",
    )
    ...
    ap.add_argument(
        "--only_ids",
        default="",
        help="Comma-separated ids to include (e.g., #001,#0421)",
    )
    ap.add_argument(
        "--ids_file",
        default="",
        help="Optional file with one id per line",
    )
    ...
    # Preflight: sanity on input schema (messages presence)
    ...
    if with_msgs == 0:
        print(
            ...
            "Use *.oracle.text.messages_v5_final.jsonl built with the oracle builder."
        )
    ...
    if args.only_ids.strip():
        allow_ids |= {s.strip() for s in args.only_ids.split(",") if s.strip()}
        ...
        with open(args.ids_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if s:
                    allow_ids.add(s)
    if allow_ids:
        items = [ex for ex in items if ex.get("id") in allow_ids]

    out_path = Path(args.out_jsonl)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    # Stop when sentinel appears
    ...
    with open(out_path, "w", encoding="utf-8") as wf:
        for ex in items:
            ...
            chat_msgs = build_messages_from_final_oracle(sys_txt, ex)
            ...
            wf.write(
                json.dumps({"id": cid, "raw": out}, ensure_ascii=False) + "\n"
            )
