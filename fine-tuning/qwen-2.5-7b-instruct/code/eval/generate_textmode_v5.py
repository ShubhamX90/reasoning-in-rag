...
----------------------------------------------------------------
- Uses dev.text.messages_v5_final.jsonl-style inputs:
...
Example:
  python code/eval/generate_textmode_v5.py \
    ...
    --lora_dir checkpoints/sft_qlora_v5/best_dev_f1 \
    --input_jsonl data/splits/dev.text.messages_v5_final.jsonl \
    --system_prompt_path prompts/sft/system_text_mode_sft.v5.txt \
    --out_jsonl outputs/dev_generations/sft_qlora_v5_run2.raw.jsonl \
...
def main():
    ...
    ap.add_argument("--only_ids", default="", help="Comma-separated ids to include (e.g., #001,#0421)")
    ap.add_argument("--ids_file", default="", help="Optional file with one id per line")
    ...
    print(f"[check] examples={len(items)}, with_user_messages={with_msgs}, "
          f"missing_user_messages={len(items) - with_msgs}")
    if with_msgs == 0:
        print("[FATAL] input_jsonl has no usable 'messages' with a user role. "
