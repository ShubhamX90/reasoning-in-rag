...
def build_label_C(conflict_type, conflict_reason):
    # In oracle mode we still respect the allowed set but we expect conflict_type to be gold.
...
def main():
    ...
    for ex in read_jsonl(args.in_jsonl):
        conflict_type = (ex.get("conflict_type") or "").strip()

        # Build user message (ORACLE: also pass conflict_type into the template)
        ...
        # Build assistant (TEXT-MODE, same shape as end-to-end)
        arr = build_array(ex)
    ...
    print(f"[Convert-ORACLE] {args.in_jsonl} → {args.out_jsonl}  kept={kept}, dropped={dropped}")
